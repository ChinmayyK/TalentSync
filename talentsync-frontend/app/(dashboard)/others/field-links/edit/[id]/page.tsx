...
import { saveFieldLink, useFieldLinks } from "../../store";
...
export default function EditFieldLinkPage({ params }: { params: { id: string } }) {
    const router = useRouter();
    const { fieldLinks } = useFieldLinks();
    ...
    const existingLink = fieldLinks.find(link => link.id === params.id);
    ...
    useEffect(() => {
        if (existingLink) {
            form.reset({
                labelName: existingLink.labelName,
                fieldLinkName: existingLink.fieldLinkName,
            });
        }
    }, [existingLink, form]);
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        saveFieldLink(updatedLink);
        toast.success("Field Link updated successfully!");
        router.push("/others/field-links/all");
        setIsSubmitting(false);
    }

    if (!existingLink && fieldLinks.length > 0) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[50vh]">
                <h2 className="text-2xl font-bold mb-4">Field Link Not Found</h2>
            ...
            <div className={cn(
                ...
                <div className="max-w-7xl mx-auto px-6 py-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">Edit Field Link</h1>
                            <p className="text-sm text-slate-500">Update specific field association</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-border/50 shadow-sm overflow-hidden p-6 md:p-8">
                            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-border/50">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">Edit specific field properties</p>
                                </div>
                            </div>
                    ...
                    <div className="hidden lg:block w-80 shrink-0 sticky top-28">
                        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-2xl p-6 border border-blue-100 dark:border-blue-900/50">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <h4 className="font-semibold text-blue-900 dark:text-blue-100 text-sm">Note</h4>
                                    <p className="text-sm text-blue-700 dark:text-blue-300 mt-1 leading-relaxed">
                                        Changing the Field Link Name may affect existing integrations if they rely on this specific identifier.
                                    </p>
                                </div>
                            </div>
                        </div>
